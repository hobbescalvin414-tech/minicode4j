...
import type { UiInputState } from "./input-state.js";
...
export class StreamingRenderer {
  private lastRendered: { type: "tool_started"; toolUseId: string } | { type: "other" } | null = null;

  render(event: UiEvent): string {
...
export function renderEvents(events: UiEvent[]): string {
  return renderTranscriptEvents(events);
}

export function renderTranscriptEvents(events: UiEvent[]): string {
  const finishedToolIds = new Set(
    events.filter((event) => event.type === "tool_finished").map((event) => event.toolUseId)
  );
  return events
...
export function renderEvent(event: UiEvent): string {
  return renderTranscriptEvent(event);
}

export function renderTranscriptEvent(event: UiEvent): string {
  switch (event.type) {
    case "ready":
...
export function renderLiveRegion(options: {
  detailsState: Pick<SessionDetailsState, "backendStatus" | "lastStatusText" | "recentContext">;
...
export function renderDetailsPanel(state: SessionDetailsState, inputState: UiInputState): string {
  return [
...
export function renderUserSubmittedLine(text: string): string {
  return `${BLUE}›${RESET} ${goldText(text)}`;
}

export function renderSubmittedCommandLine(command: UiCommand, rawLine: string): string | null {
...
function renderToolFinished(event: Extract<UiEvent, { type: "tool_finished" }>): string {
...
function renderLiveStatus(state: Pick<SessionDetailsState, "backendStatus" | "lastStatusText" | "recentContext">): string {
...
function renderLiveFooter(state: Pick<SessionDetailsState, "recentContext">): string {
  return state.recentContext == null ? muted("context pending") : muted(state.recentContext.badge);
}

function renderPermissionLive(state: Extract<UiInputState, { mode: "permission" }>): string[] {
  return [
    "",
    bold(state.title),
...
function permissionDisplayLabel(choice: { key: string; label: string }): string {
  switch (choice.key) {
    case "allow_once":
      return "Yes, proceed";
    case "allow_turn":
...
function renderAskUserLive(state: Extract<UiInputState, { mode: "ask_user" }>): string[] {
  return [
    "",
    bold("Question"),
    state.question
...
function renderPermissionFeedbackLive(): string[] {
  return [
    "",
...
function promptForLive(_state: UiInputState): string {
  return `${BLUE}›${RESET} `;
}

function renderDiffPreview(diffPreview: Extract<UiEvent, { type: "tool_finished" }>["diffPreview"]): string {
  if (diffPreview == null) {
    return "";
  }
...
function renderContextLine(state: SessionDetailsState): string {
...
function renderToolLine(state: SessionDetailsState): string {
...
function renderPermissionLine(state: SessionDetailsState): string {
...
function renderAskUserLine(state: SessionDetailsState): string {
...
function failedText(text: string): string {
...
function toolText(toolName: string, summary: string): string {
...
function terminalRowCount(text: string, columns: number | null): number {
  if (columns == null) {
    return 1;
  }
...
function normalizeColumns(columns: number | undefined): number | null {
