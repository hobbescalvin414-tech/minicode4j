...
import { initialSessionDetailsState, reduceSessionDetailsState } from "./session-details.js";
async function main() {
    ...
    backend.process.stderr.on("data", (chunk) => {
        stderrText += chunk;
    });
    ...
    const exitCode = await new Promise((resolve) => backend.process.on("close", resolve));
...
function attachBackendRenderer(backend, onEvent, shouldRender = () => true, writeRendered = (text) => {
    if (text.length > 0) {
        output.write(text + "\n");
    }
}) {
    ...
    backend.process.stdout.on("data", (chunk) => {
        buffer += chunk;
...
async function readInteractiveCommands(options) {
...
class LiveRegionController {
    ...
    getDetailsState;
    renderedRowCount = 0;
    renderedRowsBelowCursor = 0;
    constructor(rl, getInputState, getDetailsState) {
        this.rl = rl;
        this.getInputState = getInputState;
        this.getDetailsState = getDetailsState;
    }
    ...
    clearAfterEnter(submittedLine = currentReadlineLine(this.rl), submittedCursor = currentReadlineCursor(this.rl)) {
        ...
        this.renderedRowCount = liveRegionSubmittedClearLineCount(this.renderedRowCount, submittedLayout.terminalRowCount);
    ...
    clearAdditionalRows(nextLineCount) {
...
function currentReadlineLine(rl) {
    const line = rl.line;
    return typeof line === "string" ? line : "";
}
function currentReadlineCursor(rl) {
    const cursor = rl.cursor;
    return typeof cursor === "number" ? cursor : currentReadlineLine(rl).length;
}
function renderDetailsDuringQuestion(rl, panel, prompt) {
...
main().catch((error) => {
    stderr.write(`ts-ui error: ${error.message}\n`);
    process.exitCode = 1;
});
