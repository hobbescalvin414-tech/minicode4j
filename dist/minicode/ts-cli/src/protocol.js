export function parseJsonlEvents(text) {
    ...
    for (let index = 0; index < lines.length; index++) {
        ...
        try {
            ...
            if (error.message.startsWith("Invalid UI event at line ")) {
                throw error;
            }
            throw new Error(`Invalid UI JSONL at line ${index + 1}: ${error.message}`);
        }
    }
    return events;
}
export function toCommandLine(command) {
    return JSON.stringify(command);
}
function validateUiEvent(value, line) {
    if (!isRecord(value)) {
    ...
    switch (type) {
        ...
        case "status":
            ...
            return value;
        case "turn_stop":
            expectOneOf(value, "reason", line, ["FINAL", "AWAIT_USER", "MAX_STEPS", "MODEL_ERROR", "CANCELLED", "EMPTY_RESPONSE_FALLBACK"], "turn_stop.reason");
...
function validateDiffPreview(value, line) {
    if (!isRecord(value)) {
...
function expectChoiceArray(value, field, line) {
...
function expectStringArray(value, field, line, label = field) {
